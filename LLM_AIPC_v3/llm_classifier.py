"""LLM autoregressive likelihood classification with LoRA, QLoRA, rsLoRA, DoRA, or head-only tuning."""
...
try:
    from .llm_registry import MODEL_CONFIGS, get_llm_model_config  # type: ignore  # noqa: E402
except ImportError:
    from llm_registry import MODEL_CONFIGS, get_llm_model_config  # noqa: E402


DEFAULT_TEMPLATE = "Label:{label}\nText:{text}"


class ARClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        text = self.template.format(text=self.texts[idx], label=self.label_words[label_idx])
        ...
        item["labels"] = labels
        item["class_labels"] = torch.tensor(label_idx, dtype=torch.long)
        return item


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an LLM autoregressive likelihood classifier.")
    ...
    parser.add_argument("--label-words", default="No,Yes", help="Comma-separated label strings ordered by encoded label class.")
    parser.add_argument("--likelihood-reduction", default="mean", choices=["mean", "sum"])
    ...
    parser.add_argument("--resume-from-checkpoint", default=None, help="Path to a checkpoint directory, for example outputs/v3/qwen_qlora/checkpoint-last.")
...
def apply_model_defaults(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.model_loader is None:
        args.model_loader = config.model_loader
    base_model_lower = str(args.base_model).lower()
    if args.model_loader == "causal_lm" and ("ministral-3" in base_model_lower or "mistral-3" in base_model_lower):
        args.model_loader = "mistral3_conditional"
...
def is_baichuan_model(args: argparse.Namespace | dict[str, object]) -> bool:
...
def build_model(args: argparse.Namespace, tokenizer):
    ...
    if args.tuning_mode == "head_only":
        ...
        if args.verbose:
            print(f"trainable head params: {trainable}", flush=True)
...
def candidate_scores(
    ...
    for start in range(0, len(text_list), batch_size):
        ...
        candidates = [template.format(text=text, label=label_word) for text in batch_texts]
        encoded = tokenizer(
            candidates,
            truncation=True,
            padding="max_length",
        ...
        if reduction == "mean":
            ...
            batch_scores = -loss_sum / denom
        else:
            batch_scores = -loss_sum
...
def evaluate(
    ...
    with torch.no_grad():
        all_scores = [
            candidate_scores(model, tokenizer, texts, label_word, template, max_len, device, batch_size, reduction)
...
def save_best_model(args: argparse.Namespace, output_dir: Path, tokenizer, model, encoder, label_words: list[str], metrics: dict[str, object]) -> None:
...
def save_checkpoint(
    ...
    config["model_type"] = "llm_ar_classifier"
...
def train(args: argparse.Namespace) -> dict[str, object]:
    ...
    train_loader = DataLoader(
        ARClassificationDataset(train_texts, y_train, tokenizer, args.max_len, args.template, label_words),
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if score > best_score:
            ...
            save_best_model(args, output_dir, tokenizer, model, encoder, label_words, metrics)
        save_checkpoint(
            args,
            checkpoint_last,
            tokenizer,
